...
\item Fixing the location of the upper camera using a bit of glue. \todo{Tim: We nood to check, if this kind of rule is OK for Aldebaran.}
...
The coaching robot is not placed on the field but on the table that is used for the GameController computer. The coaching robot is not allowed to move and has to remain on its seat, which is provided by the organizers or brought by the team.  \todo{Tim: Does the coaching robot need a specific number?}
...
The coaching robot is not allowed to communicate with the robot players\todo{Tim: We need to find a way to make sure that the coach does not receive data from the robots on the field}. Instead, it sends messages to the GameController's coaching interface \todo{Tim: My idea is to use the GameController messages for transporting coaching information. This will be the easiest way. Interesting side effect: The other team can hear what the other coach says :-)}. These messages have a maximum size of N \todo{define N} bytes and will be copied by the GameController to its standard messages. However, independent of the sending frequency of a coach, the GameController will accept a new message only every M \todo{define M} seconds, all other coach messages will be dropped. In addition, the update of the coach message will be delayed by the GameController by O seconds \todo{define O}. This represents the intent that the coaching robot should make long-term strategic decisions instead of immediately controlling the robot players.
...
Each team should plan for at least one team member to attend the drop-in games.  This team member will need to update the team color and player number of their team's drop-in player between games, and referee when his or her team is not competing in the drop-in challenge. \todo{Tim: I think that the content of the last paragraph is obvious and could be removed.}
