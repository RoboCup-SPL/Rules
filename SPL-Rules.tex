...
\hline
H & Center circle diameter & 1200 \\
\hline
I & Border strip width & 700 \\
\end{tabular}
\caption{Schematic diagram of the soccer field (not to scale). and corresponding dimensions in mm.} \label{fig:field_dim}
\end{figure}
...
\caption{Dimensions of the goal, viewed from above and its placement on the field.}
...
\end{minipage}
\begin{minipage}[t]{0.49\columnwidth}
Goalposts and crossbar made from 3 yellow cylinders with a diameter of 10cm. 
The net:
\begin{itemize}
\item has a height of 80cm
\item is of white, grey or black color
\item is properly supported, in a way to minimize interference with the goal keeper
\item is attached to the field to catch the ball
...
In addition to the visible lines, there are two invisible lines only relevant to the throw-in procedure (see Section \ref{sec:throw_in}), but not relevant to the construction of the field. These two throw-in lines are 400~mm away from the outside lines and run parallel to them inside the playing area. They are centered lengthwise with respect to the field. Each throw-in line is 4~m\todo{adjust} long.

The throw in lines will be marked (as dots at the end of the throw in lines and short dashes along the line) by the technical committee at the start of competition with felt-tip pen - these lines are intended to stay invisible to robots but provide a guide to referees.
...
\item Goals~(\cf Figure~\ref{fig:goal_appearance}). The posts and top cross bar of both goals are yellow (RAL 1018). The support triangles on back of the posts and the net are white.
...
A match is played by two teams, each consisting of not more than \textbf{5} players. Maximal one of them may be designated as \emph{goalkeeper}, the others are \emph{field players}.
...
The field players are not allowed to enter their own penalty area. The three field players robots have the jersey numbers ``2'', ``3'', ``4'' and ``5''.
...
\begin{figure}[b]
  \centerline{\begin{tabular}{lll}
      a) & b) & c) \\
      \includegraphics[height=0.28\columnwidth]{figs/pink_front.jpg}&
      \includegraphics[height=0.28\columnwidth]{figs/blue_angle.jpg} &
      \includegraphics[height=0.28\columnwidth]{figs/blue_back.jpg} 
  \end{tabular} }
...
Teams can use a bandwidth of up to 500 Kbps \todo{reduce the bandwidth} of the wireless. This includes any data transferred, namely the actual payload and any protocol overhead created, \eg, by TCP, UDP, or the GameController.
...
It is up to the team's choice on which sideline and where exactly on the sidelines the robots are placed.
...
\begin{figure}[t]
\centerline{\includegraphics[width=\columnwidth]{figs/manual_placement_2012.png}}
...
The goal keeper is not allowed to touch a ball that is completely outside the penalty area, and the attacking robot is not allowed to touch a ball once it has been completely inside the penalty area. The line is part of the penalty area.
This way, a penalty shot is effectually over after the first goal shot which entered the penalty box, either in favor of the attacker if a goal has been scored, or in favor of the defending team if the goal keeper successfully blocked or if the ball just did not reach the goal line.
However, if the goal keeper touches the ball when the ball is outside the penalty area, then a goal will be awarded to the attacking team.
...
%Due to the symmetry of the field, there is no way for the referees to mis-configure one of the robots for the penalty kick, therefore a penalty kick is never repeated.
...
%Section~\ref{sec:situations_no_pushing} defines situations where pushing does not apply. Those exceptions overrule all other conditions. In all other situations, a \textit{pushing contact} between two robots is considered player pushing and penalized according to section~\ref{sec:pushing_results}. Sections~\ref{sec:pushing_contact} to~\ref{sec:pushing_fallen} define such pushing contacts between two and more walking/standing robots, and with fallen robots, respectively.
...
a robot from the team will be removed for every 2 pushing offenses (i.e., the 6th time, 8th time, 10th time, etc.). Pushing during the ready state does not come into this removal count (\cf Section~\ref{sec:kick-off}). 

If the ball moves significantly as the result of pushing, then it should be replaced to where it was at the time of the infringement. The ball movement must not result in an advantage for the pushing team. If this is not the case, the ball does not need to be replaced if, in the referee's opinion, this would create more interference with the game than not replacing it.\todo{is it needed?}
...
%If 3 robots are in contact or close to contact, and pushing occurs following the rules of Section 4.9.1, the team with more robots involved is the main offender and one of their robots will be penalized. This is meant to give an advantage to a robot of team A which fights against two robots of team B, since one robot of team B will be called for pushing first. If pushing still occurs after the removal, the situation is judged according to Section~\ref{sec:pushing_contact}.
...
The head referee and the assistant referees should wear clothing and socks/shoes \emph{of black or dark blue color}\footnote{In contrast to previous years, this includes blue jeans, as these can not be confused with blue goal posts any more.} and avoid reserved colors for the ball, the goals, and player markings in their clothing. They may enter the field in particular situations, \eg, to remove a robot when applying a penalty. They should avoid interfering with the robots as much as possible.
...
The intent of switching to same-colored goals is to remove artificial landmarks.
...
\section{Changes From 2012}

This is a brief list of rule changes from 2012 to 2013.

\begin{itemize}
	\item Increased the size of the field to $6~m\times 9~m$ (cf. Section~\ref{sec:field_dim}).
	
	\item Use the same goals construction as the humanoid kid-size league (cf. Section~\ref{sec:field_dim})
	
	\item Increased the number of players to \textbf{5} (\cf Section~\ref{sec:robot_players})
	
	
	\item Simplified the pushing rules. In particular:
	\begin{itemize}
		\item Pushing while walking front-to-back is not treated separately anymore. (\cf Section~\ref{sec:pushing_contact}).
		\item No special treatment if more than two robots are involved in pushing (\cf Section~\ref{sec:pushing_several_robots}).
		\item Removed an old rule: referees will not put upright a robot fallen after pushing, i.e., a robot hat to get up on its own (cf. Section~\ref{sec:player_pushing}).
		\item Exceptions to the pushing rules do apply in the same way to contacts with own and opponent players (\cf Section~\ref{sec:situations_no_pushing}), i.e., it's no pushing if two robots of the same team are fighting front to front for the ball.
	\end{itemize}
	
	\item If a timeout is ended prematurely, the opponent team has up to 2 minutes time to be ready to play (cf. Section~\ref{sec:request_for_timeout}).
	\item Clarified, that \emph{any} changes on the robot (hardware and software) may be made during a request for pick-up (cf. Section~\ref{sec:request_for_pickup}).
\end{itemize}
	

\section{Changes From 2011}

This is a brief list of rule changes from 2011 to 2012.

\begin{itemize}
	\item Both goals have the same color now (yellow)!
	\item Included the possibility that the head referee may relocate persons or objects around the field if they seem to be placed there as some kind of artificial landmark. See Section~\ref{sec:judgement:landmarks} for details.
	\item Robots have defined starting positions at the beginning of each half. See Section~\ref{sec:initial-kick-off} for details.
	\item A penalty kick ends after one shot on the goal, i.e. after the striker kicked the ball inside the penalty box. No further shots are allowed anymore in case the goal keeper blocks and the ball rolls out of the penalty box again. This is to increase the similarity to real soccer. However, the striker is still allowed to touch the ball more than once before performing the actual goal shot.
	\item During a penalty shoot-out,the attacking robot will be indicated by the GameController using the kick-off flag, not the team color as before (cf. Section~\ref{sec:penalty_kick}).
	\item Clarified that penalty kicks should not be repeated.
	\item Small change in the formulation of the pushing rule for easier understandability; no change in the meaning is intended. 
	\item Added a note about marking the invisible throw-in lines. 
	\item Local Game Stuck (cf. Section~\ref{sec:game_stuck:local}): The responsible robot has previously been picked up and placed at a symmetric position. In oder to avoid this without introducing new complicated special cases, this robot will now simply get the standard removal penalty.
	\item Changed the ``Request for Pick-up'' rule (cf. Section~\ref{sec:request_for_pickup}) concerning when it is allowed and what is allowed to be changed/fixed/substituted.
	\item Referees are allowed to wear blue jeans, now that those can not be confused with blue goal posts any more.
