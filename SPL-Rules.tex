...
\change{I would suggest to write it in a different way, since it is allowed to use the goalie to play on the entire field. My suggestion:
...
\item Ball holding may not occur when a robot falls on a ball.  The robot will either get-up and hence free the ball, or the robot should be removed under the Fallen Robot rule. \change{PG: At this point we do not say what happens if the ball was moved or scored a goal. I think it should be handled like in the Example of this subsection.}
\info{TW: It's already covered under rules for accidental playing with hands.}
\end{enumerate}
...
If the ball moves significantly as the result of pushing, then it should be replaced to where it was at the time of the infraction. \change{PG: Also at this point we do not say what happens if a goal gets accidental scored.}
\info{TW: As above.}
...
\begin{enumerate}
  ...
  \item A stationary robot cannot be penalized for pushing, including a robot that is kicking, provided that the ball was close enough where a kick could have succeeded at the start of the kick motion. \change{It should only apply to the kick of the robot. So if the kick of the kicking robot destabilizes the other robots, than we do not have pushing. Otherwise if the robot falls after or during kicking and than pushes it should be called for pushing.}
...
\change{Add the figures for Center Circle and penaly cross as well.}
