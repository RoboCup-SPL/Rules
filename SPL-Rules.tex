...
a robot from the team will be removed for every 2 pushing offenses, i.e., the 6th time, 8th time and 10th time. Pushing offenses during the ready state are not counted as part of this removal count (\cf Section~\ref{sec:kick-off}).
...
Each time a team fails to provide two referees for a game in which they are scheduled to provide referees, they will incur a one goal penalty in their next match.  This penalty will not carry over from one competition to another.
...
\begin{itemize}
	...
	\item Updated legal kick-off positions, as well as manual positions (cf. Section~\ref{sec:kick-off}).

	\item On kick-off, the defensive team must stay outside of the center circle until the ball is in play (cf. Section~\ref{sec:kick-off}).  The ball is in play once it is touched by the attacking team or once 10 seconds have elapsed in the playing state.
	...
	\begin{itemize}
		\item Robots are allowed to push robots on their own team (\cf Section~\ref{sec:player_pushing}).
	...
	\item For each match in which a team does not provide referees as required, the team will receive a one goal deduction in their next match (cf. Section~\ref{sec:refPenalty}).
	
	\item ``Illegal Defender'' rule is applied if a player of the defensive team enters the center circle before the ball is in play (cf. Section~\ref{sec:kick-off}).
