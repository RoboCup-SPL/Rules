...
A penalty shot is over when the ball has come to a full stop after the first contact by the attacking robot. A goal is awarded to the attacking team if a goal has been scored (\ie the ball has completely crossed the goal line), or the score is unchanged if the goal keeper successfully blocked the shot, if the ball did not reach the goal line, or if the ball leaves the field.
