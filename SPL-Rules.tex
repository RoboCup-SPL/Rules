...
Note that all robots, that were removed with the standard removal penalty do not get positioned manually but have to wait to get unpenalized.
...
The field players of the defending team can walk to any position within their own half, except for inside the center circle. Note that the "Illegal Defender" rule does apply here, as in all other states (see Section~\ref{sec:illegal_defender}).
...
\begin{enumerate}
  \item The head referee will announce a Free Kick, calling ''Pushing Free Kick \textless team\textgreater'' for the team with the pushed robot or ''Goal Free Kick \textless team\textgreater'' for the defensive team.
  \item Game Controller will activate the substate for the Pushing Free Kick or Goal Free Kick, respectively.
...
If a robot of the offending team does enter or not attempt to leave the \FreeKickRadius area around the ball after a Free Kick (see Section~\ref{sec:free_kick}) was called, ''Illegal Defender'' is called as well. This rule does not apply for the goalkeeper robot within its own penalty area.
...
\begin{itemize}
  \item Every applicant has to bring a poster containing the team's contribution, focused on the current year, to the RoboCup event to share this information with the other teams.
  \item When using another team's code base, the applicant has to replace one major\footnote{This refers to parts like vision system, localization, walking but excplicitly not to a behavior that uses the other team's code base.} part with their own code for each year the other team's code base is used. The replaced parts have to be publishable to be considered contributions that warrant a qualification.
%  \item If an applicant can not provide this required information in time for the application, he is not considered officially qualified for the RoboCup competition until he can. Provided the applicant fulfills the requirements previous to the registration deadline of the RoboCup event, the qualification decision can be withheld until that time if the applicant agrees\footnote{This ensures, that any significant contribution to the league that is ready for the RoboCup is considered.}.
\end{itemize}
...
The striker is positioned on circle segment of radius 1m and an opening angle of 120° around the penalty spot facing the ball. On this circle segment two spots to the left and two spots to the right with a distance of each 30° are marked as starting position (see Figure~\ref{fig:general_penalty_setup}). For each round each team has to hand over one striker and one goalkeeper or just one robot if goalkeeper and striker are the same robot. Afterwards the head referee throws a die to determine the starting position as in Figure~\ref{fig:general_penalty_setup} for this round).
...
\begin{itemize}
  ...
  \item Penalties during the \emph{ready} state result in a standard removal penalty instead of a manual placement and do not get manually positioned at the end of the ready phase.
\end{itemize}
...
\begin{itemize}
  ...
  \item Clarified, that only two players may enter the team's penalty area at the same time during \emph{all} game states.
\end{itemize}
...
\begin{itemize}
  ...
  \item Place the ball on the sideline after a ball out.
