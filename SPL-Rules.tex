...
The field players are not allowed to enter their own penalty area. The four field players robots have the jersey numbers ``2'', ``3'', ``4'' and ``5''.
...
If robots collide during the autonomous placement, the ``Player Pushing'' rules are applied, but the penalty is manual placement by the assistant referees, not the penalty described in Section~\ref{sec:player_pushing}.
Consequently those pushings do not go into the permanent removal count.
...
This way, a penalty shot is effectively over after the first goal shot which entered the penalty box, either in favor of the attacker if a goal has been scored, or in favor of the defending team if the goal keeper successfully blocked or if the ball just did not reach the goal line.
...
a robot from the team will be removed for every 2 pushing offenses (i.e., the 6th time, 8th time, 10th time, etc.). Pushing offenses during the ready state are not counted as part of this removal count (\cf Section~\ref{sec:kick-off}).
...
\begin{itemize}
	...
	\item Forfeits no longer result in a 10:0 score.  When an opponent forfeits, the other team must play out the game against an open field.  Any own goals scored by this team will not count, however (\cf Section~\ref{sec:forfeit}).
\end{itemize}
