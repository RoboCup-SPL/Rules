...
Robots that will not participate in the shootout (including the coach) must not be on the wifi network and must stay outside of the field.
...
\begin{itemize}
	\item Clarification of situations in which goals may be scored when the ball touches the arms or hands of a robot. (Section \ref{sec:goal})
	...
	\item Slight clarification of qualification penalties related to forfeiting a game (Section \ref{sec:forfeit})
\end{itemize}

\subsection*{Forbidden Actions and Penalties}
\begin{itemize}
	\item Added exceptions to playing the ball with arms/hands and clarified situations in which goals may be scored. (Section \ref {sec:hand_ball})
\end{itemize}
...
\begin{itemize}
	...
	\item Combined the text from "Failing to referee when assigned" with that describing referee selection and guidelines (Section \ref {sec:refSelection})
	\item Added a description of qualification penalties. (Section \ref {sec:qualificationPenalties})
\end{itemize}
...
The following rules are planned to be removed before RoboCup 2018 if insufficient teams make use of them during RoboCup 2017:

\begin{itemize}
    \item Coach robot (Section \ref{sec:coaching_robot})
    \item Goal keeper save (Section \ref{sec:goalie_save})
\end{itemize}
