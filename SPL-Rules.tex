...
The coaching robot is not placed on the field but on the table that is used for the GameController computer. The coaching robot is not allowed to move.  It may either stand on the table, sit on the table, or sit in a seat.  A seat will be provided by the organizers, but teams may bring their own seat. \todo{Katie: Should we put limitations in the seat, such that the robot can not be held to a height taller than X while sitting in the seat?} \todo{Tim: Does the coaching robot need a specific number? Katie: Perhaps for GameController?  Otherwise, I don't see why.} \todo{Katie: Should we define where the seats/standing area will be on the table?}

\subsection{Team Markers}
\label{sec:team_markers}

Robots will use colored jersey shirts as team markers. The official jersey shirt colors are cyan and magenta (referred to as blue and red throughout this document).  Each jersey shirt will have a player number(1-5) printed on it in white text.  The official jersey shirts will be the jersey shirts used at Robocup 2013.  Teams who competed in Robocup 2013 should use the jerseys they purchased at Robocup 2013.  New teams may purchase official jersey shirts by X \todo{how can they purchase jerseys?}.  Teams must purchase the official jersey shirts --- teams are not allowed to use jersey shirts that they design themselves or that are manufactured by a different supplier.  The team markers are worn as shown in Figure~\ref{fig:nao_markers}.
...
The coaching robot is not allowed to communicate directly with the robot players\todo{Tim: We need to find a way to make sure that the coach does not receive data from the robots on the field}. Instead, it sends messages to the GameController's coaching interface \todo{Tim: My idea is to use the GameController messages for transporting coaching information. This will be the easiest way. Interesting side effect: The other team can hear what the other coach says :-) Katie: Sounds good.}. These messages have a maximum size of N \todo{define N} bytes and will be copied by the GameController to its standard messages. However, independent of the sending frequency of a coach, the GameController will accept a new message only every M \todo{define M (10?)} seconds, all other coach messages will be dropped. In addition, the update of the coach message will be delayed by the GameController by O seconds \todo{define O (5?)}. This represents the intent that the coaching robot should make long-term strategic decisions instead of immediately controlling the robot players.
...
The goal keeper is allowed to hold the ball for up to 5 seconds as long as it has one foot inside in its own penalty area. In all other cases, robots are allowed to hold the ball for up to 3 seconds. Holding the ball for longer than this is ``Ball Holding'' and is not allowed.
...
Clustering should be called when two robots from opposite teams are within kicking distance of the ball and an additional robot from either team contacts one of the original robots that was within kicking distance of the ball.  The referee should remove any additional robots at the moment they touch one of the original two robots according to the standard removal penalty.

If two or more robots from a team are within kicking distance of the ball, this is allowable until a robot from the opposing team approaches the cluster of robots.  The first robot contacted by the approaching robot is removed according to the standard removal penalty.  If two or more robots from one team remain within kicking distance of the ball, these robots should also be removed until only one robot from each team remains within kicking distance of the ball.

\todo{Katie: I'm starting to have second thoughts regarding this rule, especially the second paragraph.}
...
\begin{itemize}
\item Any player may be goal keeper (ie, enter the goal box and behave as goal keeper).  However, once one player has become goal keeper for a team, this player will be the goal keeper for the remainder of the game.  Any subsequent defensive players to enter the goal box will be removed as illegal defenders. \todo{Check this after overworking the illegal defender rule}
\end{itemize}
...
\item team has no goal keeper = -3
\item walking into a teammate = -2
\item illegal defender = -2
\end{itemize}
\todo{think about the criteria}
\todo{should one judge consider each team?  or both judge both teams, and average their scores?}
...
\todo{are we okay with one of the SPL All Stars also being on the championship team?}
...
\begin{itemize}
	...
	\item The operator of the Game Controller is now officially in change of game timing. (Section \ref{sec:gameControllerOp})
