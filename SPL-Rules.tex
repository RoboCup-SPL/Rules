...
\date{(2018 rules draft, as of \today)}
...
Robots may be disentangled by the referees at the start of the Ready state. After that, penalties should be called as normal, but these penalties do not count towards the incremental penalties. 
...
\begin{enumerate}
  \item The head referee will announce a Free Kick, calling ''Penalty/Goal Free Kick \textless team\textgreater'' for the team with the pushed robot.
  ...
  \begin{enumerate}
    \item Only the team awarded with the Free Kick may approach within \FreeKickRadius to the ball.
    ...
    \item Opposition robots that violate (a) or (b) are penalized with the ''Illegal Defender'' penalty (see Section~\ref{sec:illegal_defender}) which results in a standard removal penalty (see Section~\ref{sec:removal_penalty}).
...
A Penalty Free Kick is awarded against the robot penalized for pushing if:
\begin{itemize}
...
If a robot of the offending team does enter or not attempt to leave the \FreeKickRadius area around the ball after a Free Kick (see Section~\ref{sec:free_kick}) was called, ''Illegal Defender'' is called as well. This rule does not apply for the goalie robot within its own penalty area.
\paragraph{Note:} The referee should not look for exact distances and rather penalize only those robots who clearly violate this rule. As a guideline, the robots of the offending team should clear the ball within 10 seconds.
...
\begin{itemize}
\item Post in a publicly available location a team research report describing their work for the 2018 competition
...
For the 2018 tournament, pairs of teams will announce their partnership as part of the answer to the official call for application, as in last year's application. As part of the application, potential team pairs should announce the name of the second team, the mixed team name, and their proposed jersey color. Teams are strongly encouraged to change their mixed team partner from the previous year to spread the communication and behavior advancements.

\subsection{Limitations}
In 2018, the tournament will be limited to six mixed teams. Both teams comprising a mixed team should run their own codebase, but are encouraged to develop a layer for interoperability which could later be used to form mixed teams with other teams. 
...
For the 2018 tournament, there will be 10 games in total: two round robins consisting of 3 teams each with 3 games, two semifinals, a 3rd place game, and a championship game.  Hence, each mixed team would play a total of 2 to 4 games in the mixed team tournament.
...
The previous year has shown significant improvements in penalty shootout skills. To foster this improvements and prepare for the future years this year another penalty shootout challenge will be held. The rules are a slightly modified version of the usual penalty shootout rules.

\subsection{Process of the Tournament}
The challenge will be held as a single elimination tournament as in the previous year. This means there will be:
\begin{itemize}
  ...
  \item The Final.
\end{itemize}
Moreover, it is mandatory to provide two referees per team for the challenge. The winner of the challenge will be awarded with a certificate.
...
The striker is positioned on circle segment of radius 1m and an opening angle of 120° around the penalty spot facing the defended goal. On this circle segment two spots to the left and two spots to the right with a distance of each 30° are marked as starting position (see Figure~\ref{fig:general_penalty_setup}). For each round each team has to hand over one striker and one goalkeeper or just one robot if goalkeeper and striker are the same robot. Afterwards the head referee throws a die to determine the starting position as in Figure~\ref{fig:general_penalty_setup} for this round).
...
\begin{itemize}
  ...
  \item Penalties during the \emph{ready} state result in a standard removal penalty instead of a manual placement. Those penalties however still not count towards the incremental penalty counter.
\end{itemize}
...
\begin{itemize}
  ...
  \item Extended the ''Illegal Defender'' rule due to the Free Kick rule.
\end{itemize}

\subsection*{The Official RoboCup Competition Rules}
\begin{itemize}
  \item Added an intermediate round between second round robin and quarterfinals for champions cup and challenge shield.
  \item The games between champions cup and challenge shield are now named promotion round.
\end{itemize}

\subsection*{Mixed Team Tournament}
\begin{itemize}
  \item Number of allowed mixed teams increased to 6 this year.
  \item Added a section for the penalty shootout procedure in a mixed team tournament.
\end{itemize}

\subsection*{General Penalty Kick Challenge}
\begin{itemize}
  \item Introduced a modified penalty shootout as a challenge for 2018.
\end{itemize}


\section{Future Changes}
\begin{itemize}
  \item Use a Free Kick instead of a Throw-In (''Kick-In'').
  \item Replace the setup for the penalty kick with the setup described in the ''General Penalty Kick Challenge'' from this year.
\end{itemize}
