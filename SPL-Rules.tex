...
\begin{figure}[b]
  ...
  \end{tabular} }
  \caption{Preliminary version of team markers. a) Front view. b) Side view. c) Back view.} \label{fig:nao_markers}
\end{figure}
...
Each time a team fails to provide two referees for a game in which they are scheduled to provide referees, they will incur a one goal penalty in their next match.  In other words, they will start their next match with a negative score.  This penalty will not carry over from one competition to another.
...
\begin{itemize}
	\item Increased the size of the field to $6~m\times 9~m$, and the center circle diameter to $1.5~m$ (\cf Section~\ref{sec:field_dim}).
	
	\item Use the same goals construction as the humanoid kid-size league (\cf Section~\ref{sec:field_dim})

	\item Fields may be located close to one another, with no barriers or boundaries between them.  There will be at least three meters of space between fields if there is not a boundary (\cf Section~\ref{sec:boundaries}).

	\item Increased the number of players to \textbf{5} (\cf Section~\ref{sec:robot_players}).

	\item Robots will use cyan and magenta jersey shirts as team markers (\cf Section~\ref{sec:team_markers}). 

	\item Updated legal kick-off positions, as well as manual positions (\cf Section~\ref{sec:kick-off}).  In particular, any number of the field players of the attacking team may position themselves inside the center circle before kick-off.

	\item On kick-off, the defensive team must stay outside of the center circle until the ball is in play (\cf Section~\ref{sec:kick-off}).  The ball is in play once it is touched by the attacking team or once 10 seconds have elapsed in the playing state.  The ``Illegal Defender'' rule is applied if a player of the defensive team enters the center circle before the ball is in play (\cf Section~\ref{sec:kick-off}).

	\item The attacker is allowed only one contact with the ball on penalty kicks. The attacker's starting position on penalty kicks was also updated for the larger field (cf. Section~\ref{sec:penalty_kick}).
	...
	\item Forfeits no longer result in a 10:0 score.  When an opponent forfeits, the other team must play out the game against an open field.  Any own goals scored by this team will not count, however (\cf Section~\ref{sec:forfeit}).
        ...
        \item All five of a team's robots can be removed for the remainder of a half if they are called for pushing twelve times in that half (cf. Section~\ref{sec:player_pushing}). 

	\item Simplified the pushing rules. In particular:
	\begin{itemize}
		\item A ball that moves significantly as the result of pushing should always be replaced to where it was at the time of the infringement (\cf Section~\ref{sec:player_pushing}).
		\item Removed an old rule: referees will not put upright a robot fallen after pushing, i.e., a robot has to get up on its own (cf. Section~\ref{sec:player_pushing}).
		\item Robots are allowed to push robots on their own team (\cf Section~\ref{sec:situations_no_pushing}).
		\item Pushing while walking front-to-back is not treated separately anymore (\cf Section~\ref{sec:pushing_contact}).
		\item There is no special treatment if more than two robots are involved in pushing (\cf Section~\ref{sec:pushing_several_robots}).
	\end{itemize}

	\item A penalty kick is no longer awarded when an opponent's robot presents a threat to spectator safety.  Instead, this robot is removed for the remainder of the game (\cf Section~\ref{sec:damage}).

	\item For each match in which a team does not provide referees as required, the team will receive a one goal deduction in their next match (\cf Section~\ref{sec:refPenalty}).  This will lead to the offending team starting their next match with a negative score.
