...
\emph{Rick: Do we want something like this for the goal keeper? Its meant to be something like, the goal keeper can always go for the ball in its own penalty area?} The goal keeper, whilst in its own penalty area, and moving to the ball, will not be penalized for pushing.
...
\emph{Rick: Do we want this also?}If a robot illegally pushes an opposing robot that is not pushing, and the collision causes the opposing robot to fall, the fallen robot will be picked up and replaced in an upright position at the point at which it fell.
...
\label{sec:hand_ball}
A field player or a goal keeper outside its own penalty box that intentionally touches the ball with its arms/hands in a manipulative manner (i.e. to stop the ball, to kick the ball etc.) will be removed for 30 seconds \emph{Rick: ??? for goal keeper, should this be 0 seconds penalty?} as per the standard removal penalty. In this case, the ball is to be replaced at the point where it contacted the arms/hands of the offending robot.
...
% removal penalty (see Section~\ref{sec:removal_penalty}). Collisions should
% be handled using the 'pushing' rules (see Sections~\ref{sec:goalie_pushing} and
% \ref{sec:field_player_pushing}).
%
...
% robots.  We hope that this penalty never needs to be called.
%
% \begin{figure}[t]
