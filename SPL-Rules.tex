...
\change{Would change it to: To teams of robots will change ...\newline\newline The reason would be to it previous and later sections "teams" are referenced as the humans who program but we want the robots to change the sides}
...
\unsure{Should we drop the last sentence? Because it should be left to the teams how to check this?}
...
\change{I would suggest to write it in a different way, since it is allowed to use the goaly to play on the entire field. My suggestion:
An indirect kick requires the ball is touched by \textit{two} different robots. The ball must also travel at least 0.75m from the point of restart in play for the second touch of a different robot of the same team to satisfy the indirect kick. If a team has only one robot left, it is required that a robot of the opponents team touches the ball before a goal can be scored.}
...
Robots that do not listen to the GameController will be placed manually \change{(see Figure\,\ref{fig:ko})}, if positioned legally.
...
During the \textit{set} state, the team leader may request manual placement \change{for} all robots on that team --- including those penalized in place for ``Motion in Set'' but not those with other penalties.
...
\unsure{Does it matter from which side? At GameController side or the other one?}
...
\change{Should we note that after the Penalty free Kick is complet all normal rules apply, just as a note?}
...
\unsure{Do we want to note that the playing signal we be sent 10sec delayed after whistling for those robots not listing to the whistle? What happens if the striker moves during set? Do we apply the Motion in set rule?}
...
\item Ball holding may not occur when a robot falls on a ball.  The robot will either get-up and hence free the ball, or the robot should be removed under the Fallen Robot rule. \change{At this point we do not say what happens if the ball was moved or scored a goal. I think it should be handled like in the Example of this subsection.}
\end{enumerate}
...
If the ball moves significantly as the result of pushing, then it should be replaced to where it was at the time of the infraction. \change{Also at this point we do not say what happens if a goal gets accidently scored.}
...
\begin{enumerate}
  ...
  \item A stationary robot cannot be penalized for pushing, including a robot that is kicking, provided that the ball was close enough where a kick could have succeeded at the start of the kick motion. \change{It should only apply to the kick of the robot. So if the kick of the kicking robot destabilizes the other robots, than we do not have oushing. Otherwise if the robot falls after or during kicking and than pushes it should be called for pushing.}
