...
A goal keeper that adopts a crouch position (that is a position with wide spread, but still supported only by the feet) for the penalty kick, can legally do so, but must move from or via an upright position to the crouch position. It cannot commence the penalty kick in that position. The rule on robot stance still applies, i.e. the robot can not stay in a position wider than its shoulders for more than 5 seconds.
