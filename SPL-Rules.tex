...
\item Designs must be submitted to \url{rc-spl-tc@lists.robocup.org} for approval by May 1, 2020. If the team has jersey prototypes, they should submit close-up images of a robot wearing the jersey - these images should be taken from front, back, and side angles.  If the team has no prototypes, then designs depicting the expected jersey should be submitted.  If submissions show seperated front and back halves of jerseys then the team must specify which halfves are matched to form home and away jerseys.  All images and designs should be submitted in pdf or jpg format.
\end{itemize}
...
From any restart in play (Kick-off, Kick-in, Free kick) except for Penalty kick, the attacking team may only score a goal via an indirect kick, providing they have at least 3 active robots on the field at the time the restart occurs (or is awarded in the case of a free kick).
%That is, if a team is awarded a free kick, the indirect rule applies if they had 3 active robots on the field when the head referee awards the free kick.
...
%A robot may not score a goal from an indirect kick, including via deflections.
%The ball must be deliberately played-at by another robot (from either team) before a goal may be scored.

%An indirect kick prevents the attacking robot from scoring directly from the restart.
%An indirect kick requires that the ball is played-at by \textit{two different} robots (from either team) before a goal may be scored.
...
\begin{itemize}
  ...
  \item Use spoken referee commands during gameplay. This would allow the robots to start to interpret the referee direction and reduce the dependence on the GameController computer. RoboCup Soccer will need to move further in this direction to reach the 2050 goal.
