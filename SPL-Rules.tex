...
\item For NAO versions that require a memory stick to remain in the head during operation, alternate memory sticks may be used instead of the Aldebaran supplied memory sticks.
...
If robots incur penalties during Ready state, the penalty is manual placement by the assistant referees, not the normal penalty. Consequently those penalties do not go into the permanent removal count.
...
Just after the \emph{set} state is called, the ball is placed on the center point of the center circle by one of the referees. If it is moved by one of the robots before set is called it is replaced by one of the referees.
...
When Local Game Stuck is called, the nearest robot to the ball will get the standard removal penalty according to Section~\ref{sec:removal_penalty}.
...
The main rule book describes the rules for RoboCup Standard Platform Games.  However, at the international RoboCup competition there will be a competition in the Standard Platform League in which teams are formed of players from various teams.
...
Each participating team will contribute one drop-in player.  Each drop-in player will compete in games with many different teams composed of randomly chosen drop-in players.  In each game, the opponent will also be a team composed of randomly selected drop-in players.  The exact number of games played by each drop-in player will depend on the number of teams that participate in the competition, but we expect to hold 15-20 drop-in games during the RoboCup 2016 competition.

Drop-in players will undergo a mandatory skill test, likely to be held on the second set-up day.  This test will evaluate communication as well as the ability for the player to score a goal on an empty field.  The results of this test will be used to seed players onto teams for the remainder of the tournament.  Additionally, failing to be able to score a goal within 5 minutes in the skill test (given three tries) and finishing in the bottom 25\% of drop-in players in the competition will be considered in your 2017 application.

Each participating team is required to email a short (no more than 200 words) explanation of their drop-in player strategy to \url{rc-spl-tc@lists.robocup.org} by the end of the final competition day at RoboCup 2016.  These descriptions will be released on the SPL wiki soon after the competition.
...
The four judges will judge each player on the field.  The judges will score the players based on the following criteria, although a player's judge score can be no lower than 0 if it entered the game. If a robot did not enter the game, its overall judge score will be -5 by default. This will be determined by the head judge. 
\begin{itemize}
...
How a team finishes in the Outdoor Competition will affect their seeding in the second round of the Indoor Competition.  Specifically, teams will be seeded into the second round as follows:
\begin{itemize}
	\item Teams from the first round Indoor Competition will be seeded into the second round based on their finish. This distribution will be done first and fields will be allocated based upon this distribution.
	\item One of the top two teams from each outdoor pool will be randomly distributed into each of the four second round indoor pools.  However, the random distribution must be repeated until:
	\begin{itemize}
		\item No second round pool contains two teams that finished in the top 4 at the 2015 Main Competition
		\item No second round pool contains two teams that were eliminated in the quarterfinals at the 2015 Main Competition
	\end{itemize}
	\item One of the remaining four outdoor teams will be randomly distributed into each of the four second round indoor pools.  However, the random distribution must be repeated until:
	\begin{itemize}
		\item No second round pool contains two teams that finished in the top 4 at the 2015 Main Competition
		\item No second round pool contains two teams that were eliminated in the quarterfinals at the 2015 Main Competition
	\end{itemize}
	\item The quarterfinals will be set such that - if the top four finishers from 2015 were to advance to the semifinal round - the 1st place finisher from 2015 will play the 4th place finisher and the 2nd place finisher will play the 3rd place finisher.
\end{itemize}
...
\begin{itemize}
\item Post in a publicly available location a team research report describing their work for the 2016 competition
\item Submit a shortened application as required by the call for participation for the subsequent year's competition
\end{itemize}

Teams that reach the quarter-finals in the team competition must also publicly release code from that year's codebase, either in the form of a complete release (perhaps without behavior) or limited libraries, in order to be pre-qualified for the subsequent year's competition.  This release must be documented and coded in a way where it can be used by others.
...
\begin{itemize}
	...
	\item Teams may add one sponsor or team logos to each upper leg of the robots as long as a box drawn around the non-white area of these logos does not cover more that a 25 $\text{cm}^2$ area. (Section \ref{sec:team_markers})
...
\begin{itemize}
	\item Penalties in Ready do not count towards the incremental penalty count. (Section \ref{sec:robot_states})
	...
	\item If a team leader calls for the entire team to be manually placed, even players penalized in place for ``Motion in Set'' will be manually placed. (Section \ref{sec:kick-off})
	\item The ball should be placed immediately after the Set state is called. (Section \ref{sec:kick-off})
	\item Updated the Game Stuck rules substantially.  Game stuck can be called after 30 seconds of no substantial change, where substantial change can be robots searching the field.  Global Game Stuck is called on the team further from the ball such that kick-off will go to the team that was closer to the ball. Players go into Ready state as soon as Global Game Stuck is called. (Section \ref{sec:game_stuck})
	...
	\item The first (left) team in the GameController will perform the first penalty kick. (Section \ref{sec:penalty_kick})
	\item Robot ready for penalty kicks do not need to be turned off, but should not be on the wifi network. (Section \ref{sec:penalty_kick})
\end{itemize}
...
\begin{itemize}
	...
	\item Due to the new incremental standard removal penalty, pushing will be treated like all other infractions. (Section \ref{sec:player_pushing})
	\item  A robot pushed by another robot can not simultaneously be called for pushing itself. (Section \ref{sec:situations_no_pushing})
	\item A field player (including a defender) that touches the ball with its arms/hands will be subject to the standard removal penalty and the ball is to be replaced at the point where it contacted the arms/hands of the offending robot.  If an own goal is scored as a result, the goal should count and the player should not be penalized. (Section \ref{sec:hand_ball})
	\item Jamming can also refer to infrared communication. (Section \ref{sec:jamming})
\end{itemize}

\subsection*{Drop-in Player Competition}
\begin{itemize}
	\item There will be a skill test held for drop-in player competition players.  The results of the skill test will be used to seed players. (Section \ref{sec:drop-in})
	\item Drop-in players can now be ejected for too many penalties. (Section \ref{sec:drop-in:rules})
\end{itemize}
...
\begin{itemize}
	...
	\item Teams will no longer evaluate referees and GameControllers. (Section \ref{sec:refSelection})
        \item Removed the requirement for teams to participate in the drop-in competition in order to remain pre-qualified (since main competition teams are no longer required to participate). (Section \ref{sec:preQual})
\end{itemize}
