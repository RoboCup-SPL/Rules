...
Robots can be in six different \emph{primary} states (\cf Figure~\ref{fig:robot_states}). If the wireless is available, these states will be set by the GameController. Teams must implement code to receive and correctly respond to wireless GameController packets, and also give visual indication of the game state.
...
\item If the ball was last touched by the defensive team, a \emph{Corner Kick} (\cf Section~\ref{sec:free_kick}) is awarded to the attacking team, and the ball is placed on the corner on the same side of the center field line that the ball was kicked-out.
The attacking team then proceeds with a corner kick following the standard free kick procedure (\cf Section~\ref{sec:free_kick}).

\item If the ball was touched by the offensive team, a \emph{Goal Free Kick} (\cf Section~\ref{sec:free_kick}) is awarded to the defensive team, and the ball is placed on the Goal Spot on the same side of the center field line that the ball was kicked-out. The Goal Spot is in line with the penalty spot and side-edge of the penalty box.
...
\begin{enumerate}
  \item The head referee will announce a Free Kick, calling ``Foul \textless color\textgreater \textless number\textgreater'' for the pushing robot or ``Kick-in/Goal Free Kick/Corner Kick \textless team\textgreater'' for the team that did not last touch the ball.
  \item The GameController will activate the substate for the respective free kick. Note that in the case of the Pushing Free Kick the substate is activated automatically through the ``Foul''.
  \item If the ball left the field, the ball will be positioned by the assistant referee as described in Section~\ref{sec:kick_in}.
...
Any strategic ``Request for Pick-up'' is not allowed, \ie gaining an advantage by removing the robot from the game.
...
\item A shot blocked by the goalkeeper beats a shot blocked by the goalpost which beats a wide shot. For example, if the shot of one team gets stopped by the goalkeeper and the other executes a wide shot, the first team wins.
...
If a robot is not inside its own half at the time the Set state starts, it will be penalized and removed for 15 seconds. The center line does not count as part of the own half for this penalty, although the goal box does. Illegal Positioning penalties do not follow the standard removal procedure, and hence do not count towards the incremental penalty count.
...
\paragraph{Example.} A robot holds the ball and before the referees can remove the robot, it shoots the ball into the goal. The goal will not be counted and the ball will be replaced where the robot held the ball.
...
In 2019, the tournament will start with a round robin phase of two groups with three teams and three games each. Mixed teams will be seeded into groups alternately according to their sum of Glicko ratings. After the round robins, the first and second teams from both groups play semi-finals. The winners of the semi-finals proceed to the final, while the losers play a 3rd place game. Thus, there will be 10 games in total and each team has two or four games.
...
\begin{itemize}
  ...
  \item Only penalties besides motion in set applicable in set state are now fallen robot and Illegal Positioning.
  ...
  \item A timeout resets the clock to the beginning of the current stoppage of play.
...
\begin{itemize}
  ...
  \item Fallen/Inactive Robot is not an incremental penalty.
  \item Scoring goals for oneself with the arms/hands during a fall or get-up motion is not possible.
\end{itemize}

\subsection*{The Official RoboCup Competition Rules}
\begin{itemize}
  \item Added repeating the head referee calls to the GameController operator duty list.
  \item Pre-Qualification by technical challenges is possible.
  \item Use Glicko rating for seeding.
\end{itemize}

\subsection*{Mixed Team Tournament}
\begin{itemize}
  \item Updated the allowed team number.
\end{itemize}
