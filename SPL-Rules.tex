...
Note that a goal can never be awarded where the last contact of the ball with a robot was by the arm or hand (see Section~\ref{sec:hand_ball}).
...
\item Judged constantly during game:
  ...
  \item Penalty = -1
  \item Game participation = -2 -- +2 (capped at -10 / +10 per half; needs justification by judge)
...
Throughout each half, judges may assign each robot rewards or punishments between -2 and +2 for its participating in the game.  If the judge notes that a robot is positioning well or otherwise actively contributing to the game, it can receive up to +2 for that instance.  If a robot is positioning poorly, behaving in a harmful or useless manner, or is inactive, the judge can give it up to -2 for that instance.  Any scores require a short explanation (this both justifies the score and will help the TC adapt the rule set for future competitions).

Additionally, each judge may also assign each robot an additional score between -2 and +2 at any point during a half for actions that are beneficial or harmful for the team but are not covered by the other rules.  Note that players should usually not be rewarded for merely scoring a goal, as this will be rewarded via goal differential.  Any scores require a short explanation.
...
\begin{itemize}
   \item Added the coaching robot. The coaching robot may only communicate via the SPLCoachMessage packet.  Updates of the coach message will be delayed by the GameController by about 3--6 seconds. (Section \ref{sec:robot_players})
	...
	\item Only grey Aldebaran NAO robots may be used. (Section \ref{sec:hardware})
	\item NAO robots with actuated hands may be used, but the fingers may not be used for any purpose and should remain with the fingers bent. (Section \ref{sec:hardware})
...
\begin{itemize}
	...
	\item A goal will not be awarded when a team scores on themselves and there are no opponent robots on the field that are active.  In this case, the goal is not scored and a global game stuck is called on the active team. (Section \ref{sec:goal})
	\item Note that a goal can never be awarded where the last contact of the ball with a robot was by the arm or hand. All such instances will be regarded as `Playing with Arms/Hands' (see Section \ref{sec:hand_ball}).  (Section \ref{sec:goal}).
	\item While in its penalty box, a goal keeper is able to save the ball by picking up the ball using atleast one arm/hand and holding it for at least 1 second.  The ball is then placed on one of the two intersections of a throw-in line and the halfway line.  Specifically, the ball will be moved to the intersection point closer (regarding the angle) to the goal keeper at the moment of lifting.  If the goal keeper is almost aligned with the field's x-axis, the referee should use the intersection point at which a teammate is closer at the moment of lifting. (Section \ref{sec:goalie_save}) 
	\item Small Xs will be marked on the field to specify manual positions. (Section \ref{sec:kick-off})
...
\begin{itemize}
	...
	\item A robot is within the penalty area if any part of its body is touching the ground inside the penalty box or touching one of its lines. (Section \ref{sec:illegal_defender})
