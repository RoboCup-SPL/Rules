...
\newcommand{\PenaltyFreeKickTime}{15 seconds\xspace}
\newcommand{\PenaltyFreeKickSetupTime}{30 seconds\xspace}
\newcommand{\PenaltyKickTime}{15 seconds\xspace}
...
The colors of the soccer field are shown in Figure~\ref{fig:field_color}. Some items on the RoboCup field are color-coded:
...
The defensive team must stay outside of the center circle until the ball is in play.  The ball is in play once it is touched by the attacking team or once \emph{10 seconds} have elapsed in the playing state.
...
This denotes that the robot are given time to setup and prepare for the penalty kick. 
For \textit{all} competition games the game clock is \textit{paused}.
The referee should also pick up the ball.
...
The attacking team have \PenaltyFreeKickTime to complete the Penalty Free Kick.
...
The striker (attacking) robot will be indicated by the GameController using the kick-off flag.
...
For penalties that are timed, the penalty time is considered to be over at half-time.
...
\subsection{Referee Calls \& Whistle Sounds}
\label{sec:referee-calls-whistles}

Referees and GameController operators should make efforts to use consistent and clear calls.
It is preferrable for Referees to use the calls as specified in these rules.\footnote{The calls specified in these rules are detailed in English. With the agreement of the teams, the referees may use suitable calls in any language. The exception to this are technical challenge that depends on the calls as specified.}.
...
\begin{itemize}
  ...
  \item Goalkeeper robot must remain on the goal line and standing.Penalty Free Kick procedure introduced~(\cf Section~\ref{sec:penalty_kick})
  \item Rules streamlined~(\cf Section~\ref{sec:penalty_shoot-out})
  \item Streamlined Sudden-death shoot out procedure~(\cf Section~\ref{sec:sudden_death_shoot_out})
\end{itemize}
...
\subsection*{Referee Signals, Spoken Calls \& Whistle Sounds}
\begin{itemize}
  \item Defined Referee Spoken Calls~(\cf Section~\ref{sec:referee-calls-whistles})
  \item Defined Referee Whistle Sounds~(\cf Section~\ref{sec:referee-calls-whistles})
\end{itemize}
