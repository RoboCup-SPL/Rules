...
Robots will use colored waistbands as team markers. Official waistbands are blue and pink and have been distrubited to each team. An example of how the team markers are worn is shown in Figure~\ref{fig:nao_markers}.
...
\subsection{Keeper Stance}
\label{sec:keeper_stance}

Goal keeper robots are not allowed to stay in a stance that is wider than the width of the robot's shoulders for more than 5 seconds. The robot is allowed to go into a wide stance as long as it comes back to a normal stance within 5 seconds. Staying in a wide stance for longer than 5 seconds will result in the standard 0 second keeper penalty. This rule is to prevent teams from using stationary keepers that stay in a wide blocking stance. 
...
\item Two robots facing each other with the ball in between them cannot be penalized for pushing.

\item Two robots moving towards the ball that make shoulder-to-shoulder or arm-to-arm contact cannot be penalized for pushing.

\item A robot within 50 cm of the ball, that is moving towards the ball in an attempt to make a play on it cannot be penalized for pushing.
...
The standard removal penalty will apply for pushing. \emph{After the 3rd time a team is called for pushing in a half, the offending robot will be removed for the remainder of the half.} If the ball is moved as the result of pushing, then it will be replaced to where it was at the time of the infringement.
...
\item Forbid keeper robots from staying wide stance for more than 5 seconds (Sec.~\ref{sec:keeper_stance}).
\item After 3 team pushing penalties in one half, the team's robot will be removed for the remainder of the half (Sec.~\ref{sec:pushing_results}).
