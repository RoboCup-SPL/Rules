...
\todo{Eric: What should the referees do if the problem lasts longer than 10 minutes?}
...
\todo{Eric: the judging appears to reward offense far more than defense (assuming passing occurs in either situation). Also some of the terms are vaguely defined - e.g. would kicking the ball out count for "clearing from own penalty area", it is easy to see how a "pass" could be interpreted very differently by different people, etc.}

The two metrics will be normalized and added up to determine the overall winner of this competition: \todo{Eric: perhaps for individual robot we throw out its best game performance and its worst game performance (Katie: both in judged and goal differnetial?)}
\begin{itemize}
