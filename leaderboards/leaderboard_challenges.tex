...
The orientation of the obstacle robots will be determined by the TC before the challenge starts, and will be the same for all teams and runs.
...
All positions are as indicated in \cref{fig:walk_leaderboard}, that is with the goal closest to the competing robot
on the right side of it's starting position.
...
starting position to the target touchline, while weaving around the obstacle robots. The competing
robot must pass on the "outside" of the obstacle robots. From the perspective of the starting position
that is to the left of obstacles on the penalty mark and corner of the penalty area, and to the right
of the obstacles on the goal area corners.

(see \cref{fig:walk_leaderboard} for an example)
...
The competing robot will be placed in the centre of the goal line facing the ball
\begin{figure}[t]
...
complete in a given timeframe, emphasising passing accuracy, speed, and coordination.
...
1.5 metres of the field (Centre Circle Diameter) is an exclusion zone, no robots are allowed within this area,
...
The ref will announce each time the ball crosses the exclusion zone, and whether this will
...
\subsection{Scoring}
1 point is awarded every time the ball crosses the exclusion zone,
with an additional point awarded for a "successful pass". A "successful pass" is defined as when
the ball stops in an 180◦ arc with radius 75cm in front of the receiving robot.

No points are counted if the time runs out before a robot has possession of the ball
