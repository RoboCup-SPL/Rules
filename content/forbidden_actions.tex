...
\cbp{Since some teams are playing with robots from other teams, there is a maximum of \textbf{two} hardware related penalties for each robot in the first half and \textbf{one more} in the second half. Hardware related penalties are:}
\cbp{\begin{itemize}
...
\subsubsection{Damage to the Robot}
...
Illegal positions are described below.
...
During the \textbf{whole} game only \textit{three} players (including goalkeeper) from the \textit{same} team can be in the \textit{same} penalty area at the same time. This means a total of 6 robots may in the same penalty area at the same time.
...
A fallen robot which is unable to autonomously stand up within 2 attempts, or 3 if disturbed externally during the first stand-up attempt, \cbw{and be at least 10 seconds upright} will be penalized and removed for \qty{45}{\second}.
...
In order to intercept kicked balls the robots are allowed to go into a wide stance as long as it comes back to a normal stance within \qty{5}{\second} \cbw{and does not excessively damage the robots}\footnote{\cbw{``excessively'' is decided by the head referee in order to limit damages to robots}}. Staying in a wide stance for longer than \qty{5}{\second} will result in the standard removal penalty.
