...
\subsection{Hardware}
\label{sec:hardware}
All teams must use black, gray, red, blue, or orange plated NAO humanoid robots \cbw{in version 6} manufactured by SoftBank Robotics.
...
Each of the four field players has a jersey number from the set $\{2, 3, 4, 5\}$. %, 6\}$.
%However, by default, the number ``6'' should only be used for a substitute that enters the game later.
\\\cbw{There will be no 6th replacement robot}.
...
\cbp{If a participating team wants to use their own jerseys (\eg because of sponsors logos) and has sent them to the hosting arena all efforts should be made so that these jerseys can be used complying the followed guidelines.}

\cbp{Each team must designate a ``home'' color and an ``away'' color at least on the setup days of the GORE 2022. The team leaders can discuss the jersey assignment before the game, especially for the remote teams. If no consensus is reached then the following applies:} 
Teams must wear their ``home'' jerseys when they are ``home'' (the first team listed on the schedule). Teams will wear their ``home'' jersey when they are ``away'' (the second team listed on the schedule) as well, unless either the head referee or the GameController program believes the jerseys of two competing teams are too similar.  In this case, the ``away'' team will then wear their ``away'' jersey.
...
\begin{itemize}
  ...
  \item Adding sponsor or team logos to the upper legs of the robots (\cf \cref{fig:sponsor}). A box drawn around the non-white area of these logos must not cover more than a \qty{25}{\square\centi\metre} area. At most one logo may be attached per leg - if you wish to attach more than one logo per leg, email the \cbw{organizers of the GORE} at least two weeks before the competition. Depending on the size and design of the logos, this may be allowable.
...
The amount of data transmitted by a team in a single game is limited. The limit is measured as the total number of UDP packets sent by any robots of a team. A team may not exceed \qty{\TeamMessageLimit}{packets} per game for a game that is exactly \qty{20}{\minute} long.
