    ...
    \subsubsection{Rules}
        ...
        \paragraph{Robot Pool}
            ...
            \begin{hintbox*}{Robot usage}
                ...
                All teams an encouraged to implement features in their code to prevent falling or when falling to fall less destructive. We ask all teams to exchange ideas on that in our \textit{RC-SPL Discord}.
            \end{hintbox*}
        
        \paragraph{Robot pool disqualification}
            All teams using robots for the robot pool are encouraged to implement robot protection features to prevent abnormal damages and abnormal probability of a fall robot. The teams providing robots trust in the teams using their robots that they operate them with care.

            If a team providing a pool robots used in a game thinks that their robot has not been operated with affordable care, this team is allowed to request an ``Investigation'' by the head referee after the game.

            All team captains of the used robots by the team under investigation, the captain of the team under investigation and the head referee meet for the investigation directly after the game. The head referee is leading the investigation. The team which requested the investigation presents their arguments why they think their robot got not used with care. Then all other teams can add arguments or express their view. Last the team under investigation can give their arguments.
            When all arguments have been exchanged, the following options are available:

            \begin{enumerate}
                \item The requesting team takes back their request. No further direct consequences for the team been under investigation. But the team is asked to improve their usage to prevent any future investigations.
                ...
                \item The team under investigation disagrees with the arguments of the requesting team: The robot providing teams now vote secretly for giving a penalty card by just telling their decision to the head referee. The head referee makes the decision public to the team under investigation. If the majority of the teams is for a penalty card, the team under investigation gets a penalty card for robot pool usage. 
            \end{enumerate}

            A team getting their first penalty card has to improve their code for the pool robots until the next game to prevent future damages. They have to announce their modifications to the teams providing next their pool robots. These teams are allowed to request a pick-up of their robot during the game, if they think that their robot has not been handled carefully. They should give the team, the opportunity to change code, to solve the problem. If after a change, the problem is not solved, they are allowed to pick the robot up again for the reminder of the game. With this pick up they have to request an ``Investigation''.
            If a team with a first penalty card, does not get another penalty card in the following game, the card get removed.
            If a team gets a second penalty card, it is not allowed to use a pool robot in their next game. After their next game, one penalty card gets removed.
            If a team gets a second time a second penalty card, they are disqualified from the robot pool for the reminder of the competition.
