...
The team which scores the most goals is the winner of the game. If the two teams scored the same number of goals, the game will be a draw. A drawn game may be decided by a penalty shoot-out (see Section~\ref{sec:penalty_shoot-out}).
\todo{TW: Added here since this really should be in the Game process.}
...
\unsure{Challenge Shield Rule: Section~\ref{sub:cc-initial-kick-off}}
The first kick-off at the start of each half is the initial kick-off.
Before the initial kick-off, \ie before the start of each half, all robots must be in the \textit{unstiff} state and must be placed on the sidelines in their own half of the field, by the referees according to \cref{fig:initial_positions}.
Once the robots receive the \emph{ready} signal from the GameController, they are to proceed as described in \cref{sec:kick-off}.
...
\subsection{Kick-in}
\label{sec:kick_in}
\unsure{Challenge Shield Rule: Section~\ref{sub:cc-kick-in}}
...
\label{sec:free_kick}
\unsure{Challenge Shield Rule: Section~\ref{sub:cc-free-kick-procedure}}
...
\begin{itemize}
  ...
  \item A pushing penalty (see \cref{sec:player_pushing}) awarded near the ball against the defending team within their own penalty area, termed a \textit{Penalty Kick}.\unsure{Challenge Shield Rule: Section~\ref{sub:cc-fk-penalty-kick}}
\end{itemize}
...
\unsure{Challenge Shield Rule: Section~\ref{sub:cc-fk-penalty-kick} and Section~\ref{sub:cc-penalty-kick-procedure}}
...
Additionally, only the goalkeeper robot from the defending team may be within the penalty area. The goalkeeper robot must also be touching the goal line.
...
\unsure{Challenge Shield Rule: Section~\ref{sub:cc-request-for-pickup}}
...
Any forfeit will result in a qualification penalty being recorded (\cf \cref{sec:qualificationPenalties}) but the circumstances of the forfeit will affect the severity of the offence and the impact on future qualification.
...
Referees place the ball, the striker, and goalkeeper robots. The ball is placed on the penalty spot closest to the goal being defended. The striker robot is positioned on the edge of the penalty area, facing the ball and the goal. (This striker position is denoted by a small dot made with a felt-tip pen.) The goalkeeper is placed with its feet on the goal line and in the center of the goal.
...
The goalkeeper robot must always be in contact with the goal line, is not permitted to leave the goal area, and must remain on its feet until the striker robot touches the ball.
The goalkeeper is only permitted to ``dive'', and be off it's feet after the attacking robot has touched the ball.
Furthermore, the goalkeeper is not allowed to touch a ball that is completely outside the penalty area (the line is part of the penalty area).
