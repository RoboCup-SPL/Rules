...
The half-time break is \qty{10}{\minute}, and during this time both teams may \cbw{change programs}, or do anything else that can be done within the time allotted. \cbw{But there will be no replacement of robots!}
...
\begin{description}
  ...
  \item[Playing.] In the \emph{playing} state, the robots are playing soccer. Shortly pressing the chest button will switch the robot to the \emph{penalized} state. During the \emph{playing} state, the GameController can activate the substates for free kicks (\cf \cref{sec:free_kick}).
...
\begin{figure}[t]
    ...
    \\From any state it can be transitioned to the \textit{unstiff} state, shown as a blue arrow, via pressing all \textbf{three head buttons}. Pressing the \textbf{chest button} in the \textit{unstiff} state allows a transition to the initial state, or a state as indicated by the GameController.}
...
If a robot plays the ball to itself, this means that the ball must leave a circular area of at least \qty{0.75}{\metre} radius around the robot before the ball is played a second time and to be considered as an indirect kick.
...
The attacking team can be positioned anywhere within their own half and one robot is allowed to be positioned inside the whole center circle without blocking its center.
The defending team can be positioned within their own half, except for inside the center circle including its line.
No player is allowed to touch the halfway line except for the player within the center circle.
...
The defensive team must stay outside of the center circle until the ball is in play. The ball is in play once it is touched by the attacking team or once \emph{\KickOffBallFreeTime} have elapsed in the playing state. Before the ball is in play, robots are not allowed to leave their own halves. Otherwise, a robot will be penalized with the ``Illegal Position'' penalty~(\cf \cref{sec:illegal_positioning}).
...
Each team can call a \textbf{maximum of 1 timeout per game} with a total time of no more than \textbf{5 minutes}. During this time, both teams may \cbw{change robot programs}, or anything else that can be done within the time allotted. \cbw{But there will be no replacement of robots!}\\
\cbp{Since the teams are not on site, the assistant referees take over these tasks as if they were a member of the corresponding team.}

During normal game time, a team may call a timeout at any stoppage of play (after a goal, stuck game, before a half, etc.). Alternatively, a team may call a timeout before a penalty shootout if they have not used their timeout yet (\cf \cref{sec:penalty_shoot-out}).
...
Note that there are a few unlikely cases that are not covered by these rules.  If a situation is not covered by these rules, the \cbw{organizers of the GORE} and the organizing committee will work together to make a decision.
