...
For RoboCup 2014, the Technical Committee identified five possible technical challenges, which are described in this document. Out of these five, two or three challenges will be selected based on the vote of all SPL teams that intend to participate in RoboCup 2014 (\ie that submit qualification material). For this purpose, an online poll is open at \todo{Add URL} XXX until \todo{Add Deadline} YYY.
...
Each robot will be given 3 minutes to walk on the field. If a robot falls over and its get up motion fails on a particular surface, the team can request to relocate the robot at the previous starting position.
