...
The carpets are arranged with increasing (expected) difficulty. \todo{I assume the TC will determine the expected difficulty?  This could vary based on each walk.  Perhaps we want to have the easiest on either end (if we allow the teams to replace their robot at either end after falling)?}
...
\todo{Is there a time limit for this challenge?  3 minutes?  Or perhaps an allowed number of falls?}
\todo{When a robot falls, must it get up on its own?  Or we could allow teams to place it standing in either penalty zone.}
\todo{Perhaps it would be better to structure the challenge such that each team gets X attempts to walk on each carpet.  Points could be awarded in a similar fashion, but this would eliminate the possibility of a carpet that a team could walk on in the middle of the course that they may never reach due to struggling with the `easier' carpets.}
