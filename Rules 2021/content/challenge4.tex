...
% use push button interface
% Reference standard SPL message corordinates
% could just be voice?
% if there is a broken arm on the robot, then should be reported and skipped (or derefer to voice only)
% Refer to challnge 3 explicity for field dimensions - also remove field locatliation challenge
% remove dimensions not be exact
% refer to full documentation
% check that istances are mesaured by the referee
% host venue needs measuring tape
% be clear on marking robot lcoation with smaller sticker to measure at the end
% maybe stop time?

% head + button standard signal to enter calibration mode
% repeat challenge w/ head sensor
% use fron/back to distinguish

% could use the log file co-ordinates?
