...
This challenge tests a team's automatic calibration software and separated from the 1v1 challenges to allow teams to demonstrate the quality of their calibration tools.
...
The Prequeqistes and Setup for this challenge will follow the same process as described in Sections~\ref{sec:c3_Prerequisites} and~\ref{sec:arean-org-setup} for the ``1 vs 1 Challenge``, with the following important changes:
\begin{enumerate}
...
This challenge will be conducted before teams calibrate their robots for the 1v1 challenge. If this is not possible, the challenge will be conducted the first time that a team set's up their robots for use at a given venue.
...
\begin{enumerate}
    ...
    \end{itemize} 
    \item If the 5 minute period is reached, the robot should immediately stop. The robot may also return to a seated position and untiffen~(see~\ref{sec:robot_states}). If it does not do so the referee may use the chest button interface to penalise the robot (informing the robot to stop calibration). 
