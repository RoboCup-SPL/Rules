\section{Remote Challenge 3: 1 vs 1}
...
\subsubsection{Start: 2h before match}
    ...
    \begin{enumerate}
        ...
        \begin{itemize}
            \item with the standard Softbank binary image and the LAN IP address is set to 10.1.TEAM\_NO.2 and the replacement robot to 10.1.TEAM\_NO.3, if no individual image is provided by the team.
        ...
        \item put on jerseys 2 (player) or 3 (substitute)
    \end{enumerate}
...
\subsubsection{Calibration / Testing: 1 hour before match)}
    ...
    \begin{enumerate}
        ...
        \item For autonomous setup is conducted in the following way: The volunteer placed the robot on the normal initial position at the field side. Then the front head button plus the chest button gets pressed to initialize auto calibration mode. The robot than can move over the whole field to calibrate itself. The robot has to finish its calibration within 5 minutes and sit down and gets un-stiff. Otherwise the robot will un-stiff the robots by pressing all head buttons simultaneously. 
...
\info[]{Penalty spot position as in 2019}
...
Each field player has a jersey number from the set $\{1, 2, 3, 4, 5, 6\}$. However, by default, the number ``2'' is used for the first field player and the number ``3'' should be used for a substitute that enters the game later. This assignment can be changed due to availability of jerseys. \improvement[]{TODO: forbid jersey nr 1 to avoid confusion?}
...
The head referee signals a goal by a single whistle blow, followed by the call ``Goal \textless colour\textgreater''. However no GameController action shall be performed. \improvement[]{TODO: Maybe we can change the logic of the GameController to allow for counting goals without ready and set phase.} \\
...
\improvement[]{TODO: Allow manual placement or simplify the rules without it?}
...
\improvement[]{TODO: Single GameController button for Global Game Stuck?}
\improvement[]{Global game stuck starts with Initial position}
...
\improvement[]{TODO: Are the Assistants allowed to fix hardware problems (\ie reboot)?}
...
-\improvement[]{TODO: Allow wide stance to intercept a ball?}
...
\improvement[]{TODO: Integrate the determination/execution of a penalty for the remainder of the game if the robot has fallen multiple times?}
...
\end{itemize}
and then the points will be counted by the operator of the GameController \improvement[]{TODO: either by writing it down or by using a modified GameController}.

For teams playing with an \textbf{autonomous player} (\cf Section~\ref{sec:c3_Prerequisites}) a multiplication factor of \textbf{2} \improvement[]{TODO: Which factor seems appropriate? 2, 1.5, 1.25?} is given. This factor is used at the end of the game and the scored points, for this team, are multiplied by it to calculate the final points (\cf Section~\ref{sec:rankings}). \emph{Note that the resulting points should be rounded to the nearest integer.}
...
\improvement[]{TODO: Final determination? Coin toss?}    
