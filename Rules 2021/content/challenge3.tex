...
\begin{itemize}
    ...
    \item A team must be able to host an arena. If not, they have to find a substitute team. Substitute teams take over the hosting arena responsibilities.\change{Das sollte geändert werden, sodass andere Teams Roboter für ein anderes Team bereitstellen/schicken können}
    \item A robot should be able to semi- or fully automatic calibrate itself.
\end{itemize}

If you cannot fulfil all requirements, but you would like to participate, please contact the TC.\change{Generell ist dieser Abschnitt ja doppelt und dreifach schon gesagt worden oben oder?}
...
\begin{itemize}
    ...
    \item At least \textbf{two} working NAOs V6 per game\change{Kann weg wegen dem Robotpool}
    \item All robots assigned for the whole competition are numbered from \textbf{1} to the maximum number with integers.
    \item More than \textbf{two} tiny USB Sticks (\cf section \ref{sec:c3_USB_Drive}) with 16 GB or more memory for flashing and logging\change{am besten 10 USB Sticks}
    \item At least \textbf{four} standard balls\change{da braucht man ja jetzt nur noch einen}
...
\begin{itemize}
    ...
    \item In \textbf{Arena Access} remote teams find an instruction how to access the arena. Each team must provide team specific credentials and share them with the responsible person in each team until the 1st of June\change{Zeit anpassen}. Every team has to announce a responsible person for credentials and remote setup until the 1st of June\change{Zeit anpassen}. The list of responsible people can be found in the root folder of the Nextcloud drive.
	...
	\item Logs and images will be uploaded from the hosts into the folder \texttt{Logs/Game\_ID}, where \textit{Game\_ID} is replaced by the game identifier, after the game when bandwidth is available. It is assumed that logs and images are stored on the USB drive attached to the NAO in the folder \texttt{logs}\change{home/nao/logs oder nur logs?}. Other files will be ignored. 
