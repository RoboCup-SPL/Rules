...
This sections describes the setup procedure for a 1 vs. 1 match. The setup procedure distinguishes between fully-autonomus and semi-autonomous setup. The used setup procedure is relevant for scoring the game (\cf section \ref{sec:scoring}).
...
\subsubsection{Start: 2h before match}
    ...
    \begin{enumerate}
        ...
        \item The competing team informs the competition venue if they are using fully-autonomus or semi-autonomous setup.
		\item For each team one randomly selected functional robot is assigned from the pool
        \item Connect the robot to a charger. If the competing team chose semi-autonomous setup, a LAN cable is connected as well.
        \item Each robot gets flashed 
        \begin{itemize}
            \item with the standard SoftBank binary image. % TODO do they get flashed with both, one after the other? Or is this a or-statemnt?
        ...
        \item Teams can set their robot up remotely, if necessary. To reduce the amount of data transferred to set the robot up, teams should reduce this as much as they can and use images. If the competing team chose fully-autonomous setup, this step is skipped.
...
For teams playing with an \textbf{fully autonomous player} (\cf Section~\ref{sec:c3_Prerequisites}) a multiplication factor of \textbf{1.5} is given. This factor is used at the end of the game and the scored points, for this team, are multiplied by it to calculate the final points (\cf Section~\ref{sec:rankings}). 
