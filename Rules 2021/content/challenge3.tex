...
% Patrick
The aim of this challenge is to have competitive gameplay in a competition given the current situation.

Two robots play against each other on a full-sized or scaled-down field (\cf section \ref{sec:field_dim}). The goal is to shoot balls from the own half into the opponent's half or goal to score points. It is not allowed to leave the own half. Four balls, two on each side, can be shot by the robots.
