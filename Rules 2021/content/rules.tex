...
\cbp{Fields that are at least 3/4 of the full size can be used for the 2021 competition. That is, the field dimensions should be at least 6m x 4m. If teams do not have a field of at least 3/4 size for participation in local challenges, they should report this by email to the TC (\url{rc-spl-tc@lists.robocup.org}) by June 1, 2021 to determine if suitable arrangements can be made.}
...
The lighting conditions depend on the actual competition site. SPL fields \cbw{may} be placed near or under windows where possible. Whether or not window lighting is used, ceiling lights \cbw{should} be provided as necessary to ensure that most of the field is never darker than 300 Lux (400 Lux preferred).
...
\begin{description}
	...
	\item[Calibration.] \cbw{\textit{This state has been added in 2021.}} This state denotes the robot is acting with automatic calibration. This state may only be entered from Initial/Ready by pressing the \textit{front} head button plus the chest button.
