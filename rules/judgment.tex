...
Those circumstances are goals (\cf~\cref{sec:goal}), transitions from \texttt{set} to \texttt{playing} (\cf~\cref{sec:kick-off,sec:penalty_kick,sec:pso_kick}), and the ends of each half~(\cf~\cref{sec:game_struct}).
...
The operator of the GameController sits at a PC in the technical area (\cf~\cref{sec:technical_area}).
As with the head referee, the operator should make efforts to use consistent and clear calls.
They will enter the decisions by the head referee into the program as they are announced, so that the game state is updated.
