...
The head referee should talk to the assistant referees before the game starts to determine what tasks or lesser decisions they wish to relegate to them, however the final calls must come from the head referee.
Examples include, assistant referees keeping track of indirect-kick rule compliance by a team in the event the head referee needs confirmation if a valid goal has been scored, indicate or to suggest a call for a specific violation (leaving the field, or motion in set) of the rules committed by the robots, to which the referee can act on accordingly.
