...
In the event that there has been a referee error or a pause in game, and if a goal has not been scored by either team, the drop-ball rule shall apply. When resuming the game, the process should be the same as a normal start, however, none of the teams will be given kickoff, and no team would be allowed in the center circle until the \texttt{playing} state as there is no longer time allotted for kickoff. The only main example currently where the drop-ball rule would be required is when a referee timeout has been called with no goal being scored.
