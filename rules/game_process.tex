...
\begin{itemize}
  ...
  \item If a team is awarded a free kick but fails to execute it within the allotted time, the other team may score directly as long as the second team plays the ball first.
\end{itemize}

In both cases, the direct goal is only allowed if the kicking robot only touched the ball once.
\unsure{Discuss what to do with accidental deflections}
If any robot (whether the same or a different one) plays the ball again, then the indirect kick rule returns into effect.
In this case, the contact made by the kicking robot counts towards the indirect kick rule.
...
For most of the free kicks in Champions Cup, the referee communicates which team is to take the free kick via a visual gesture.
In this case, the GameController packet will tell the robots which free kick sub-state is active, but not who is to kick the ball.
...
This gesture is made from \textcolor{red}{\textbf{[a specified position, or anywhere along the field border]}} and held for \textcolor{red}{\textbf{[no less than 2 seconds, and possibly more]}}.

The above rule applies to the following free kicks: \textcolor{red}{\textbf{[list of set plays this applies to]}}.

For all other free kicks, and for all free kicks in Challenge Shield, this visual signal is not performed and the kicking team is communicated by the GameController.
