...
In the event that there has been a referee error or a pause in game and has been acknowledged by the head referee, and if a goal has not been scored by either team since the last kick-off play, the drop-ball rule shall apply.
The game must stop and will start again when the arising issue has been resolved.

When resuming the game, the process should be the same as a normal start, placing the robots on the sidelines.
...
The only current situation where the drop-ball rule would be required is when a referee timeout has been called with no goal being scored since kick-off.
...
If a penalty was incorrectly applied to a robot and acknowledged by the referee to be incorrect, the robot should first be removed and placed outside the field, following the same process for the Standard Removal Penalty. Once placed, the robot should then be allowed to continue play immediately after being placed and the GameController removes its penalty.
