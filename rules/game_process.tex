...
Upon reaching the \texttt{set} state, the robots must not move until the kick-off is signaled by the referee (\cf~\cref{sec:robot_states,sec:motion_in_set}).
The following restrictions apply during the \texttt{set} state until the ball becomes free:
\begin{itemize}
...
All robots outside the own half, except the closest to the center mark, are illegally positioned.
...
If a different robot of the kick-off team is executing the kick-off, this robot is considered to be illegally positioned and a moved ball gets moved back.
  \item The defending team can be positioned within their own half excluding the center circle.
  \item Robots are allowed to be positioned inside their own goal. All other positions that are fully on the border strip are illegal for both teams.
  \item Both teams are also subject to restrictions on the goal area (\cf~\cref{sec:ip_own_goal_area}).
\end{itemize}
All robots that do not reach legal positions or enter illegal positions before the ball is in play will be penalized with the ``Illegal Position'' penalty~(\cf \cref{sec:ip_kick_off}).

A referee places the ball on the center mark. If the ball is moved by one of the robots during \texttt{set}, it is moved back by one of the referees.
...
\qty{\PlayingDelayTime}{\second} after the head referee has signaled the kick-off, the robots' state is switched to \texttt{playing} by the GameController (regardless of when the ball becomes free).
