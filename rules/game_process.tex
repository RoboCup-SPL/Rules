...
In the rare event that there has been a referee error or a pause in game, [WIP] .

\subsection{Early Stopping of Match}
\label{sec:early_stopping}

A game will conclude early if the ball comes to a complete stop outside of the penalty area, nearing the end of the game.
...
If a penalty was incorrectly applied to a robot and acknowledged by the referee to be incorrect, the penalty must be aborted and a signal must be sent from the GameController, with the affected robot being unable to move, with the same process used for a Standard Removal Penalty.

The robot should then be removed and placed outside the field, following the same process for the Standard Removal Penalty. Once placed, the robot should be allowed to continue play immediately after being placed.

\subsection{GameController Audio Output}
\label{sec:gc_audio_output}

After each decision being made by the head referee, the GameController must reflect that decision and should output the decision via audio.
