...
No robot may block ball placement by standing on the center mark.
Only one robot of the kick-off team is allowed to stay outside the own half.
All robots outside the own half, except the closest to the center mark, get penalized by ``Illegal Positioning''~(\cf \cref{sec:illegal_positioning}).
If the kick-off team has a robot positioned outside the own half, only this robot, called kick-off player, is allowed to execute the kick-off.
...
When reaching the \texttt{set} state, the defending team can be positioned within their own half excluding the center circle.
...
In the \texttt{set} state, the robots must not move~(\cf \cref{sec:robot_states}). A referee places the ball on the center mark. If the ball is moved by one of the robots during \texttt{set}, it is moved back by one of the referees.
