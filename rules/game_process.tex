...
The attacking team may only score a goal via indirect kick from any restart into \textit{playing} from a free kick except for a penalty kick.
...
The head referee will announce a free kick is completed, by ``Ball Free'', and the GameController resumes the game state \emph{playing}. Note that the sub-state will be automatically left (returning to the \textit{playing} state) after the \qty{\PenaltyKickTime}{\second} time period expires.
