...
\subsection{Indirect Kick Variant - Challenge Shield only}
\label{sec:indirect_kick_old}

This rule aims to allow \textbf{Challenge Shield} teams to play games even in the unfortunate case where
severe circumstances make them unable to field enough robots to play by the
default indirect kick rules.

Before the beginning of each half during a Challege Shield game, teams declare whether
they are going to play normally or play in "reduced mode" (temporary name).
If at least one team plays in "reduced mode", the whole game follows different,
more lenient rules for indirect kicks.

\subsubsection{"Reduced Mode" rules}
If a team plays in "reduced mode", they are only allowed to field at most two robots.
The other robots must be removed from the field via "Request for Pickup".

Teams decide which "mode" to play in before the beginning of each half.
Furthermore, during any team's timeout or referee timeout,
each team may choose to switch from "normal mode" to "reduced mode", but not vice versa.
In both cases, teams are encouraged to make this decision as early as possible.

Changing "modes" during normal gameplay is not allowed.

\info[inline]{
  Besides preventing strategic abuses of the rule, having so few opportunities to change "mode"
  is also intended to help referees, ensuring that the indirect kick rules don't
  suddenly change during normal gameplay.
}

The GameController communicates to the robots the "mode" each team plays in,
...
\subsubsection{Indirect Kick in "Reduced Mode"}
As long as at least one team plays in "reduced mode", \textbf{both teams}
...
plays in "reduced mode" are as follows:
...
The head referee is encouraged to seek the acknowledgement of the GameController operator and/or look at the field-facing monitor in order to determine the timing.

The referee makes an effort to send the signal standing as close as possible to the T-junction joining the half-way line to the side line opposite to the GameController.
However, their actual positioning along the sideline may deviate slightly from this standard position, based on the current game situation and the timing of the gesture.
