...
The attacking team may only score a goal via indirect kick from any restart into \textit{playing} from a free kick.
...
A goal may be scored directly from a kick-off against the opposing team. If the ball goes directly into the goal of the player taking the kick-off, a corner kick is awarded to the opposing team.
...
\begin{itemize}
  ...
  \item \qty{\KickOffBallFreeTime}{\second} have elapsed in the playing state. 
\end{itemize}
...
The head referee will announce a penalty kick is completed by calling ``Ball Free'', and the GameController resumes the game state \emph{playing}. Note that the sub-state will be automatically left (returning to the \textit{playing} state) after the \qty{\PenaltyKickTime}{\second} time period expires.
