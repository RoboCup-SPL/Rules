...
\begin{description}
  \item[Unstiff.] It helps to facilitate a consistent and safe handling of the robots for remote competition.
    ...
    The GameController will activate this state before \texttt{visual} (i.e at the beginning of a half and during a timeout).
    Shortly pressing the chest button will switch the robot to the \texttt{penalized} state.

    \item[Visual.] The robots are not allowed to move in any fashion besides initially standing up and moving their heads.
    In \texttt{visual}, the robots are awaiting a visual indication from the referee which signals the transition to \texttt{ready}.  
  ...
  \item[Ready.] In this state, the robots walk to their legal positions for kick-off (\cf \cref{sec:kick-off}) or a penalty kick (\cf \cref{sec:penalty_kick}).
    ...
    \begin{itemize}
      \item Receiving a message from the GameController. 
      \item Detecting the whistle after a goal. 
      \item Detecting the visual signal.
      \item Receiving a message from teammates who detected the visual signal.
    \end{itemize}
...
At the scheduled time, the referee will announce that the transition from the \texttt{initial} to \texttt{visual} will be executed 60 seconds later.
The referee shall call ``Start in 60 seconds''
The GameController operator shall start the countdown through the GameController.
The GameController operator will announce the transition from \texttt{initial} to \texttt{visual} when the head referee is calling ``Start''.
If all humans from playing teams have left the field, the head referee can announce the transition earlier.


Once the game controller operator has confirmed the \texttt{visual} state, the referee should wait a random time between 10 to 120 seconds before announcing the transition to \texttt{ready}. 
Teams should expect that the delay before the referee signals the transition from \texttt{visual} to \texttt{ready} will be different every time.
The referee will announce the transition from \texttt{visual} to \texttt{ready} state by raising both hands over their head.
...
The GameController signal for the transition from \texttt{visual} to \texttt{ready} will be delayed by \qty{\SetDelayTimeChampion}{\second} for Champions Cup and by \qty{\SetDelayTimeChallenge}{\second} for Challenge Shield.
