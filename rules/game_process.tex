...
\begin{itemize}
  ...
  \item If a team is awarded a free kick but fails to execute it within the allotted time, the other team may score directly as long as the latter team plays the ball first.
    ("Execute" is intended as "kick and move the ball clearly", as per section \cref{sec:free_kick})
\end{itemize}
...
In Champions Cup, the referee communicates which team is to take the free kick via a visual gesture.
The GameController packet will tell the robots which free kick sub-state is active, but not who is to kick the ball.
\unsure{This gesture is entirely tentative and just my (Francesco's) proposal, pending discussion with the TC. The intent is to mirror the FIFA gestures in a simplified way.}
...
\textit{(For reference, the gesture is the same as the "Kick-in" gesture from the 2023 Visual Referee Challenge, but the opposite arm is raised with respect to back then.)}
This gesture is made by the head referee and held for at least 5 seconds after the GameController signal is sent.
\textit{(The referee may look at the GameStateVisualizer monitor to check that the GC signal is sent and when the time allotted for the gesture has passed.)}
The referee makes an effort to sends the signal standing as close as possible to the T-junction.
However, their actual positioning along the sideline may deviate from this standard position, based on the current game situation and the timing of the gesture.
\info{Remind the refs that the GC operator will take a few moments to click the button, giving them a little time to make a couple steps if needed.}

In Challenge Shield, this visual signal does not need to be performed and the kicking team is communicated by the GameController.
\textit{(In accordance with the teams, the referee may still perform the gesture if e.g. a team wants to experiment with recognition, without changing the GC behavior.)}
...
In Champions Cup only, the GameController does not communicate the identities of the attacking and defending team.
Instead, this information is communicated via the same visual gesture described in \cref{sec:free_kick} for free kicks.
