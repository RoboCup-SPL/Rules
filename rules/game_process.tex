...
If a team plays in \textit{fallback mode}, they are only allowed to field at most \textbf{\MaxRobotsInFallback} robots.
...
\unsure{tentative} Teams are not required to scan the entire touchline:
in broad terms, a robot standing about \textcolor{red}{\textbf{(three? four?)}} meters away from the T-junction should be able to find the referee in its camera field with no or minimal head motion.
