...
\begin{itemize}
  ...
  \item If a team is awarded a free kick but fails to execute it within the allotted time, the other team may score directly as long as the latter team plays the ball first. ("Execute" is intended as "kick and move tha ball clearly", as per section \cref{sec:free_kick})
\end{itemize}
...
The referee sends the signal standing as close as possible to the T-junction, but their actual positioning along the sideline may deviate from this standard position, even significantly,
e.g. if the referee needs to better observe a specific situation during the game.

In Challenge Shield, this visual signal is not performed and the kicking team is communicated by the GameController.
