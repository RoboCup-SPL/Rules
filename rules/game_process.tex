...
The head referee should make both of these whistle sounds from the T-junction of the halfway line.

\subsection{Game States}
\label{sec:game_states}

The game can be in \textit{five} different \textit{primary} states.
% TODO: "primary" is never explained.
These states are managed by the GameController, which sends the current state to the robots as specified in~\cref{sec:wireless}.
% TODO: Mention already here that the state is not always the true state?

Teams must implement code to receive and correctly respond to wireless GameController packets.
If at least two robots on both teams have problems with the wireless network or GameController connection, the head referee should issue a referee timeout (\cf~\cref{sec:referee_timeout}).
If fewer robots do not respond to the GameController, then they are not included in the game (via a `Request for Pick-up', \cf~\cref{sec:request_for_pickup}), and the game starts without the offending robots.

The primary states are:
\begin{description}
  \item[Initial.] This state is active before each half, during timeouts (\cf~\cref{sec:request_for_timeout,sec:referee_timeout}) and before a penalty kick shoot-out (\cf~\cref{sec:penalty_shoot-out}).
    The robots are not allowed to be moving in any fashion besides initially standing up.
    The game proceeds with an initial kick-off (\cf~\cref{sec:initial-kick-off}) or the first penalty kick (\cf~\cref{sec:pso_kick}).
  ...
  \item[Ready.] In this state, the robots walk to their legal positions for kick-off (\cf~\cref{sec:kick-off}) or a penalty kick (\cf~\cref{sec:penalty_kick}).
    The state continues until the head referee decides that there is no significant progress, up to a maximum of \qty{\KickOffAutoTime}{\second} for a kick-off and \qty{\PenaltyKickSetupTime}{\second} for a penalty kick.
    The game then proceeds to the \texttt{set} state.
    The GameController indicates whether this state is part of a kick-off or a penalty kick.

  \item[Set.] In this state, the robots stop and wait for kick-off (\cf~\cref{sec:kick-off}) or a penalty kick (\cf~\cref{sec:penalty_kick,sec:pso_kick}).
  ...
  \item[Playing.] In the \texttt{playing} state, the robots are playing soccer.
    The GameController indicates whether this state is part of a kick-off, a penalty kick, or free kick (\cf~\cref{sec:free_kick}).

  \item[Finished.] This state is reached when a half is finished, as well as after each penalty kick in a penalty kick shoot-out (\cf~\cref{sec:pso_kick}).
\end{description}

All states except for \texttt{playing} are considered to be \textit{stoppages of play}.
During these, the game clock usually does not count down.
However, the competition rules may define that the clock still counts down during the \texttt{ready} and \texttt{set} states except for the first kick-off in each half (\eg after a goal).
% comment: This deliberatly says "first kick-off in each half" and not "initial kick-off" because an initial kick-off could also be after a timeout.
% However, the kick-off after the timeout *does* count as playing time in preliminary games.

As some game state transitions, in particular goals (\cf~\cref{sec:goal}) and transitions from \texttt{set} to \texttt{playing} (\cf~\cref{sec:kick-off,sec:penalty_kick,sec:pso_kick}), are signaled by a whistle blow to which the robots should react, the GameController does not always report the true state.
In these cases, the GameController continues to send the game state as if the event had not occurred for up to \qty{\PlayingDelayTime}{\second}.
The game state switches to the true state earlier if another event occurs that could not have happened in the previous state.
Note that although the end of each half is also signaled by a whistle blow, this currently does not have any meaning for the robots.

\subsection{Robot States}
\label{sec:robot_states}

Usually, all robots should behave according to the game state.
However, there are situations in which a robot's state differs.
% TODO: This doesn't sound good.
The individual robot states are:
\begin{description}
  \item[Unstiff.] After booting, the robots are in their \texttt{unstiff} state.
    It helps to facilitate a consistent and safe handling of the robots for remote competition.
  ...
  \item[Calibration.] This state denotes the robot is acting with automatic calibration.
    ...
    It does not occur during regular games.
    % TODO: move to "remote" section?
\end{description}

While robots are put into the \texttt{penalized} state by the GameController, the \texttt{unstiff} and \texttt{calibration} states can only be reached by pressing buttons on the robot.
If a robot is in those states, it should ignore GameController messages.
An overall transition diagram of the robot states (including game states that are global for all robots) is given in \cref{fig:robot_states}.
...
The current robot state must be displayed on the LED in the torso using the following colors:
\begin{itemize}
...
The game state sent to the robots will maintain the state before the goal for up to \qty{\GoalScoredDelay}{\second} (\cf~\cref{sec:game_states}).
Alternatively, the game can finish immediately due to the \textit{mercy rule} (\cf~\cref{sec:mercy_rule}).
...
\begin{enumerate}
  ...
  \item When a team scores on themselves if there are no opponent robots on the field that are active (a definition of \emph{active} is given in \cref{sec:fallenrobots}).
\end{enumerate}
...
\begin{itemize}
  ...
  \item In the half-time break both teams switch the sides of the field of play for the other goal.
...
The game state sent to the robots will maintain the \texttt{set} state for up to \qty{\PlayingDelayTime}{\second} after the whistle blow (\cf~\cref{sec:game_states}).
...
The GameController operator and head referee will indicate this by the call ``Ball Free''.
...
Free kicks are not considered to be stoppages of play, \ie it is not possible to take a timeout during a free kick and the game clock does not stop.
...
The referee signals that the penalty kick commences by blowing the whistle once, and calling ``Playing''.
The game switches to the \texttt{penalty kick} sub-state of the \texttt{playing} game state, and the game clock is resumed.
The game state sent to robots will maintain the \texttt{set} state for up to \qty{\PlayingDelayTime}{\second} after the whistle blow (\cf~\cref{sec:game_states}).
...
\begin{itemize}
  \item The attacking team touches and clearly moves the ball, even if the robot has fallen.
...
Once the penalty kick is completed, the attacking robotThe attacking robot (taking the penalty kick) may only score a goal with the kick the completed the penalty kick.
...
Note that the restrictions on the attacking robot still apply after the penalty kick timed out (\ie if the first kick of the attacker is after the ...).
...
Once the referee calls Global Game Stuck, the game switches to the \texttt{ready} state, and a new kick-off is awarded to the team that was closer to the ball when the Global Game Stuck was called.
...
% TODO: This previously related to the "robot" state, i.e. penalized robots could be picked up.
% This should still be the case, but I don't know how to write this elegantly.
...
A team may call a timeout at any stoppage of play.
Alternatively, a team may call a timeout before a penalty shoot-out if they have not used their timeout yet (\cf \cref{sec:penalty_shoot-out}).
