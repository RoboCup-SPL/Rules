...
The operator of the GameController enters the referee call into the program, which will send the appropriate signal to the robots indicating the infraction committed.
...
When the robot is on the field again, the operator of the GameController removes the penalty from the robot. % this was "will send the \texttt{playing} signal to it."
...
The head referee will call ``Ball Holding \textless robot\textgreater'', and the robot will be removed under the standard removal penalty.
The ball should be removed from the possession of the robot and placed where the penalty occurred.
If the robot that held the ball has moved the ball before the robot can be removed, the ball shall be replaced where the penalty occurred, too.
...
The head referee will call ``Player Stance \textless robot\textgreater''.
...
The head referee will call ``Playing with Hands \textless robot\textgreater''.
