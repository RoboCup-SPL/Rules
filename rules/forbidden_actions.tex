...
If a robot begins to move during \texttt{standby} (\ie before the head referee does the visual signal), all robots from that team will be penalized until the end of the \textit{ready} state.
The head referee will call ``Forbidden Motion in Standby\textless team\textgreater''.
Any response to a false signal of any kind will also result in this penalty.
...
Any response to a whistle from another field will also result in this penalty.
