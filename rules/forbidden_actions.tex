...
If a robot begins to move during \texttt{standby} (\ie before the head referee does the visual signal), all robots from that team will be penalized until the \textit{ready} state is send.
This means the Champion Cup will be penalized until the end of the \textit{ready} state, while the Challenge Shield will face penalties until 10 seconds remain in the \textit{ready} state.
